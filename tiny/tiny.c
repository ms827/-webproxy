...
void doit(int fd) {
  ...
  printf("%s", buf);
  sscanf(buf, "%s %s %s", method, uri, version);     // 읽어들인 HTTP 요청에서 요청 방식(method), 요청 URI(uri), HTTP 버전(version) 등을 파싱
  if (strcasecmp(method, "GET") && strcasecmp(method, "HEAD")) {                   // 요청 방식이 GET이 아닌 경우, 에러를 반환
    clienterror(fd, method, "501", "Not implemented","Tiny does not implement this method");
    
    return; 
  }
  
  read_requesthdrs(&rio);                       //요청 헤더 읽어들이기
  ...
  if (is_static) {
  /*
  만약 해당 파일이 정적인 컨텐츠인 경우, 파일이 읽을 수 있는 권한이 있는지 확인하고, 정적인 컨텐츠를 제공,
  is_static 변수가 1일 경우, 즉 요청된 파일이 정적인 컨텐츠인 경우, 해당 파일이 읽을 수 있는 권한이 있는지 확인,
  S_ISREG(sbuf.st_mode)는 st_mode 필드에서 파일 타입을 가져와서, 이 값이 일반 파일인지 검사,
  S_IRUSR & sbuf.st_mode는 파일 소유자의 읽기 권한을 확인. 파일 소유자의 읽기 권한이 없는 경우, 
  S_IRUSR 상수와 sbuf.st_mode 값의 비트 AND 연산 결과는 0이 되므로, if문의 조건은 참.
  */
    ...
    /*만약 해당 파일이 동적인 컨텐츠인 경우, 파일이 실행 가능한 권한이 있는지 확인하고, 동적인 컨텐츠를 제공.*/
...
void serve_static(int fd, char *filename, int filesize) {
    ...
    char *srcp, filetype[MAXLINE], buf[MAXBUF];     // srcp-요청된 파일을 메모리매핑 하는데 사용할 포인터
                                                    // filetype - 요청된 파일의 MIME유형을 저장하는 데 사용되는 문자 배열
    ...
    Rio_writen(fd, srcp, filesize);         // Rio_writen 함수는 클라이언트로 응답 본문(srcp)을 보내는 역할
...
void serve_dynamic(int fd, char *filename, char *cgiargs) {
    ...
    if (Fork() == 0) {   /* Child */                // 자식 프로세스를 생성
        ...
        /* CGI 프로그램 실행  */ 
        Execve(filename, emptylist, environ);       // execve함수를 사용하여 filename으로 지정된 서버 측 프로그램 실행
    }
    Wait(NULL);                                     // 자식 프로세스가 종료될 때까지 기다린 후 종료
}
...
int main(int argc, char **argv) {
  ...
  while (1) {                                   // 무한히 반복하여 클라이언트의 요청을 받고 처리합니다.
    ...
    connfd = Accept(listenfd, (SA *)&clientaddr,
                    &clientlen);                //클라이언트의 요청이 들어오면, 해당 요청을 받아들이고, 연결 소켓의 파일 디스크립터를 connfd에 저장
    Getnameinfo((SA *)&clientaddr, clientlen, hostname, MAXLINE,   // getnameinfo : 소켓 주소에서 호스트 이름을 추출하여 IP 주소와 연결
    port, MAXLINE, 0);                          // 클라이언트의 호스트명과 포트번호를 가져와 hostname과 port에 저장, 유효하면 1 않으면 0
    printf("Accepted connection from (%s, %s)\n", hostname, port);
    doit(connfd);                               // doit 함수를 호출하여 클라이언트의 요청을 처리합니다.
    Close(connfd);                              // Close(connfd);: 연결 소켓을 닫습니다.
  }
}